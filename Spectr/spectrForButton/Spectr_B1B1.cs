...
{
    class Spectr_B1B1 : Spectr_numeric
    {
        public Spectr_B1B1(long[,] osob, int b1, int nudft) : base(osob, b1, nudft) {
        ...
        public override void Set_Diffrence()//
        {
            ...
            {
                if ((osob_s[3, i + 1] - osob_s[3, i]) > 0)
                {
                    diffrence[i - 1] = osob_s[3, i + 1] - osob_s[3, i];
                    diffrence_2[i - 1] = osob_s[3, i + 1] - osob_s[3, i];
                }
            }
        }
        /// <summary>
        /// Рассчитать мощьность спектра
        /// </summary>
